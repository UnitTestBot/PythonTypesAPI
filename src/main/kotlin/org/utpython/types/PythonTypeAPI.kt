...
import org.utpython.types.general.*
...
val pythonAnyType = org.utpython.types.createTypeWithMembers(org.utpython.types.PythonAnyTypeDescription, emptyList())
val pythonNoneType = org.utpython.types.createTypeWithMembers(org.utpython.types.PythonNoneTypeDescription, emptyList())

fun createPythonUnionType(members: List<UtType>): UtType =
    org.utpython.types.createTypeWithMembers(org.utpython.types.PythonUnionTypeDescription, members)

fun createOverload(members: List<UtType>): UtType =
    org.utpython.types.createTypeWithMembers(org.utpython.types.PythonOverloadTypeDescription, members)

fun createPythonTupleType(members: List<UtType>): UtType =
    org.utpython.types.createTypeWithMembers(org.utpython.types.PythonTupleTypeDescription, members)

fun createPythonTypeAlias(initialization: (UtType) -> UtType): CompositeType =
    CompositeTypeCreator.create(0, org.utpython.types.PythonTypeAliasDescription) { self ->
...
fun createPythonConcreteCompositeType(
    ...
    memberDescriptions: List<org.utpython.types.PythonDefinitionDescription>,
    isAbstract: Boolean,
    initialization: (CompositeTypeCreator.Original) -> CompositeTypeCreator.InitializationData
): CompositeType =
    CompositeTypeCreator.create(
        numberOfParameters,
        org.utpython.types.PythonConcreteCompositeTypeDescription(name, memberDescriptions, isAbstract),
        initialization
    )
